...
# Ask the user for the host's name and save it to a variable
host_name = input("Please enter the host's name: ")

# Begin a continuous loop that should only be exited when the user says they're 
# done with entering guests.
while True:

    # Ask the user for the guest's name and save it to a variable
...
party_guests = [(guest["party_number_adults"], guest["party_number_children"]) for guest in guests]
...
guest_info = [f"{guest['guest_name'].title()}: {guest['party_number_adults'] + guest['party_number_children']} guests" for guest in guests]

# Print the results
print(f"Party Guests: {party_guests}")
