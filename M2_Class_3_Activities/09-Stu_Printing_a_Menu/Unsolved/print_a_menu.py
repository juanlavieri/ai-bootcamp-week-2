...
def display_menu(menu_items, indent=""):
    for item, price in menu_items.items():
        if isinstance(price, dict):
            print(f"{indent}{item} - {item.capitalize()}")
            display_menu(price, indent + "    ")
        else:
...
while True:
    ...
    # Check if the customer's input is a number
        ...
        if int(menu_category) in menu_items.keys():
            ...
            # Call the display_menu function to print menu items
            display_menu(menu[menu_category_name])
